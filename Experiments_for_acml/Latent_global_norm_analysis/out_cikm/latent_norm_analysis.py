...
def run(name_pixel, path_pixel, name_latent, path_latent, img_size,
    ...
    # ---- (2)+(3) LATENT space: unnormalized AND normalized ----
    print(f"[{name_latent}] latent-space pass...")
    lat_un = ScalarMoments()
    lat_no = ScalarMoments()
    sp_un = SpatialMoments()
    sp_no = SpatialMoments()
    per_ch_un = None  # accumulate per-channel scalar moments via dict
    ...
    report["latent_normalized"] = lat_no.summary()

    plot_hist_cdf(lat_un, "Latent space (unnormalized)", "#d62728",
                  osp.join(outdir, "2_latent_unnorm_dist.png"))
    plot_hist_cdf(lat_no, "Latent space (normalized by global std)", "#2ca02c",
                  osp.join(outdir, "3_latent_norm_dist.png"))
