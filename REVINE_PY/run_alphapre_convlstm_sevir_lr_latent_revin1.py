...
class Runner(object):
    ...
    def _train_batch(self, batch):
        ...
        revin_mean = torch.mean(frames_in, dim=1).unsqueeze(1)
        std_val = frames_in.std()
        frames_in = (frames_in- revin_mean)/std_val
        frames_out = (frames_out- revin_mean)/std_val
    ...
    def _sample_batch(self, batch, use_ema=False, vis_diff=False):
        ...
        revin_mean = torch.mean(radar_input, dim=1).unsqueeze(1)
        std_value = radar_input.std()
        radar_input = (radar_input-revin_mean)/std_value
        radar_pred, *_ = sample_fn(radar_input,compute_loss=False)
        radar_pred = (radar_pred*std_value) +revin_mean
