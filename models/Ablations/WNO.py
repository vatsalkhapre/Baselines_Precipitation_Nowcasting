...
class WaveConv3dBTCHW(nn.Module):
    ...
    def forward(self, x):
        ...
        # ------------------------------------------------------------------
        coeffs = wavedec3(x, wav, level=eff_level, mode='periodization')
        # coeffs[0]       : approx tensor  (B, C_in, t', h', w')
        # coeffs[1..level]: list of dicts with keys aad,ada,add,daa,dad,dda,ddd
        #                   each value shape (B, C_in, *, *, *)
...
class AmpCell(nn.Module):
    def __init__(self, t_in, t_out, dim, hidden_dim
        ...
        self.wno = WaveConv3dBTCHW(dim, dim, 1, ['T', 'H', 'W'])
