...
class WaveletGaborBlock(nn.Module):
    ...
    def __init__(self, t_in, t_out, dim, num_blocks,sparsity_threshold, hidden_size_factor,
        ...
        # --- SRST block REMOVED (ablation: Wavelet + Gabor + MLP, no SRST) ---
        self.viz_counter = 0
    ...
    def forward(self, x):
        ...
        # No SRST block: use fused wavelet reconstruction directly + Gabor residual
...
class WaveletLASTOCastForecaster(nn.Module):
    ...
    def predict(self, frames_in, frames_gt=None, compute_loss=False):
        ...
        if compute_loss:
            mse_loss = self.mseloss(xas, frames_gt)
