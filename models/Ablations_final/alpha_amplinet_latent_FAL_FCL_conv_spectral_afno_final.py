...
class SpectralBlock_2D(nn.Module):
    """
    ABLATION 3a: No AFNO (coarse spectral processing removed).
    Only depthwise conv (fine) + pointwise conv (channel mixing) kept.
    """
    ...
    def forward(self, x):
        ...
        x_spa = self.dw_spatial(x_)       # fine spatial only
