...
class WaveletGaborBlock(nn.Module):
    ...
    def forward(self, x):
        """
        ABLATION A: LL-only temporal processing.
        - LL band: full Gabor+MLP temporal stream (T_in -> T_out)
        - HF bands: NO temporal stream — last frame (T-1) tiled T_out times
        - Gabor residual: LL gabor processed normally;
                          HF gabor = same last-frame tile (no HF stream)
        """
        B, T, C, H, W = x.shape
        ...
        # ============================================================
        # 2a. LL: full temporal stream (same as original)
        # ============================================================
        ll_t = rearrange(ll, '(b t) c h w -> b c h w t', t=T)
        ll_gabor, ll_mlp, ll_fused = self.stream_ll(ll_t)
        # ll_fused: (B, C, T_out, H_ll, W_ll)
        # ll_gabor: (B, C, H_ll, W_ll, T_out)

        # ============================================================
        # 2b. HF: last frame tiled T_out times — no stream applied
        # ============================================================
        # For IDWT fused path we need: (B*T_out, C, 3, H_i, W_i)
        # For IDWT gabor path we need: (B*T_out, C, 3, H_i, W_i)
        # Both use the same last-frame tile since no HF temporal processing

        hf_fused_for_idwt = []   # for fused IDWT (same as gabor IDWT here)
        hf_gabor_for_idwt = []   # for gabor residual IDWT

        for i in range(len(hf_list)):
            # hf_list[i]: (B*T, C, 3, H_i, W_i)
            hf_all  = rearrange(hf_list[i], '(b t) c n h w -> b t c n h w', t=T)
            hf_last = hf_all[:, -1]                           # (B, C, 3, H_i, W_i)

            # Tile T_out times → (B*T_out, C, 3, H_i, W_i)
            hf_tiled = hf_last.unsqueeze(1).expand(-1, self.t_out, -1, -1, -1, -1)
            hf_flat  = rearrange(hf_tiled, 'b t c n h w -> (b t) c n h w')

            hf_fused_for_idwt.append(hf_flat)
            hf_gabor_for_idwt.append(hf_flat)   # same: no HF gabor stream
        ...
        # LL: gabor output processed normally
        # HF: last frame tile (no HF gabor stream)
        # ============================================================
