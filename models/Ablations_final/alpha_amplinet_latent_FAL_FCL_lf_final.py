...
class WaveletGaborBlock(nn.Module):
    ...
    def forward(self, x):
        """
        ABLATION B: HF-only temporal processing.
        - HF bands: full Gabor+MLP temporal stream (T_in -> T_out)
        - LL band: NO temporal stream — last frame (T-1) tiled T_out times
        - Gabor residual: HF gabor processed normally;
                          LL gabor = same last-frame tile (no LL stream)
        """
        B, T, C, H, W = x.shape
        ...
        # ============================================================
        # 2a. LL: last frame tiled T_out times — no stream applied
        # ============================================================
        # For IDWT fused path we need: (B*T_out, C, H_ll, W_ll)
        # For IDWT gabor path we need: (B*T_out, C, H_ll, W_ll)
        # Both use same last-frame tile since no LL temporal processing

        ll_all  = rearrange(ll, '(b t) c h w -> b t c h w', t=T)
        ll_last = ll_all[:, -1]                               # (B, C, H_ll, W_ll)

        # Tile T_out times → (B*T_out, C, H_ll, W_ll)
        ll_tiled      = ll_last.unsqueeze(1).expand(-1, self.t_out, -1, -1, -1)
        ll_recon      = rearrange(ll_tiled, 'b t c h w -> (b t) c h w')
        ll_gabor_flat = ll_recon   # same: no LL gabor stream

        # ============================================================
        # 2b. HF: full temporal streams (same as original)
        # ============================================================
        hf_fused_for_idwt = []   # for fused IDWT
        hf_gabor_for_idwt = []   # for gabor residual IDWT
        ...
        for i in range(len(hf_list)):
            ...
            # hf_fused: (B, C*3, T_out, H_i, W_i) — same as original, rearrange for IDWT
            hf_fused_flat = rearrange(hf_fused, 'b (c n) t h w -> (b t) c n h w', n=3)

            # hf_gabor: (B, C*3, H_i, W_i, T_out) — same as original, rearrange for IDWT
            hf_gabor_flat = rearrange(hf_gabor, 'b (c n) h w t -> (b t) c n h w', n=3)

            hf_fused_for_idwt.append(hf_fused_flat)
            hf_gabor_for_idwt.append(hf_gabor_flat)

        # ============================================================
        # 3. IDWT reconstruction (fused path)
        # ============================================================
        reconstructed = self.idwt((ll_recon, hf_fused_for_idwt))

        # ============================================================
        # 4. IDWT reconstruction (gabor residual)
        # LL: last frame tile (no LL gabor stream)
        # HF: gabor output processed normally
        # ============================================================
        gabor_residual = self.idwt((ll_gabor_flat, hf_gabor_for_idwt))
