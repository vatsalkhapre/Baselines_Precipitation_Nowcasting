"""
Incremental Ablation — Step 1: MLP Only
No Gabor, No Wavelet, No conv_spectral.
...
class TemporalBlock(nn.Module):
    """
    Step 1: Single MLP applied at full resolution.
    No Gabor, no wavelet decomposition.
    """
    def __init__(self, t_in, t_out, dim, size_factor=1.0, **kwargs):
        super().__init__()
        self.t_in  = t_in
        self.t_out = t_out
    ...
    def forward(self, x):
        ...
        out  = self.mlp(x_t)                         # (B, C, H, W, T_out)
        out  = rearrange(out, 'b c h w t -> b t c h w')
        return out


class WaveletGaborBlock(nn.Module):
    """Step 1 wrapper — same interface as full model for drop-in compatibility."""
    def __init__(self, t_in, t_out, dim,
        ...
        self.t_in  = t_in
        self.t_out = t_out
        self.temporal = TemporalBlock(t_in, t_out, dim, size_factor)
