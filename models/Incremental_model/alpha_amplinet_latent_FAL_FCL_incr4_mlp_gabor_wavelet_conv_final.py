"""
Incremental Ablation — Step 4: MLP + Gabor + Wavelet + Conv + Channel Mixing
Adds depthwise conv (fine local processing) + pointwise conv (channel mixing).
No AFNO (no coarse global spectral processing).
conv_spectral = ResneSpectralBlock x2 (dw+pw only, no AFNO) — no standalone AFNO.
"""
...
class SpectralBlock_NoAFNO(nn.Module):
    """
    Step 4: dw_spatial (fine) + pw (channel mixing) only. No AFNO.
    Input/output: (B, H, W, C)
    """
    def __init__(self, dim, k_spatial, groups=8):
        super().__init__()
        pad = (k_spatial - 1) // 2
    ...
    def forward(self, x):
        ...
        x_fused = self.norm(x_spa)
        x_fused = self.act(x_fused)
        x_fused = self.pw(x_fused)
        return x_fused.permute(0, 2, 3, 1)  # (B, H, W, C)


class ResSpectralBlock_NoAFNO(nn.Module):
    def __init__(self, dim, k_spatial, groups=8):
        super().__init__()
        self.block1 = SpectralBlock_NoAFNO(dim, k_spatial, groups)
        self.block2 = SpectralBlock_NoAFNO(dim, k_spatial, groups)
...
class WaveletGaborBlock(nn.Module):
    """
    Step 4: Wavelet + Gabor+MLP + dw_spatial conv + pw channel mixing.
    No AFNO anywhere.
    """
    def __init__(self, t_in, t_out, dim,
        ...
        # conv_spectral: dw + pw only, no AFNO
        self.conv_spectral = nn.Sequential(
            ResSpectralBlock_NoAFNO(dim * t_out, k_spatial),
            ResSpectralBlock_NoAFNO(dim * t_out, k_spatial),
        )
