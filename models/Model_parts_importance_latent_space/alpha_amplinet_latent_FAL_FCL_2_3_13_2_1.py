...
class AmpCell(nn.Module):
    def __init__(self, t_in, t_out, dim, size_factor=1.0,
        ...
        self.conv_t= nn.Sequential(nn.Conv2d(t_out, t_out, kernel_size=3,padding=1),
    ...
    def forward(self, x):
        ...
        x = self.conv(x)
        x2 = rearrange(residual, 'b t c h w -> (b c) t h w')
        x2 = self.conv_t(x2)
        x = rearrange(x, 'b (t c) h w -> b t c h w', t=self.t_out)
        x2 = rearrange(x2, '(b c) t h w -> b t c h w', c = self.channels)
        x = x + x2
        return x
