...
class AmpCell(nn.Module):
    def __init__(self, t_in, t_out, dim, size_factor=1.0,
        ...
        self.conv_t= nn.Sequential(nn.Conv2d(t_in, t_out, kernel_size=3,padding=1),
                                  nn.GroupNorm(2, t_out),
                                  nn.SiLU(),
                                  nn.Conv2d(t_out, t_out, kernel_size=3,padding=1),)
        
    def forward(self, x):
        residual = self.tmlp(x.permute(0,2,3,4,1)).permute(0,4,1,2,3)
        x2 = rearrange(x, 'b t c h w -> (b c) t h w')
        residual2 = self.conv_t(x2)
        residual2 = rearrange(residual2, '(b c) t h w -> b t c h w', c=self.channels)
