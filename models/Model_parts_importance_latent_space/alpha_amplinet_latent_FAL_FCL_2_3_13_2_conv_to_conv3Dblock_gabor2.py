...
class ResnetBlock3D(nn.Module):
    def __init__(self, dim, dim_out, groups=8, kernel_size=3, padding_mode='zeros'):
        ...
        self.block1 = Block3D(dim, dim_out, groups, kernel_size, padding_mode)
        self.block2 = Block3D(dim_out, dim_out, groups, kernel_size, padding_mode)

        self.res_conv = (
            nn.Conv3d(dim, dim_out, kernel_size=1)
            if dim != dim_out
            else nn.Identity()
        )
