...
class SpectralFusion(nn.Module):
    """
    Spectral replacement for Conv3d(2C, C, (k,1,1)).
    (B, 2C, T, H, W) → (B, C, T, H, W)
    
    Per-pixel channel reduction, but done in temporal-frequency space
    so each temporal frequency mode gets its own mixing weights.
    """
    def __init__(self, in_ch, out_ch, T, num_blocks=4):
        super().__init__()
        self.in_ch = in_ch        # 2C = 128
        self.out_ch = out_ch      # C = 64
        self.T = T
        self.num_blocks = num_blocks
        self.block_in = in_ch // num_blocks    # 32
        self.block_out = out_ch // num_blocks  # 16
        self.freq_T = T // 2 + 1               # 11 for T=20

        scale = (self.block_in * self.block_out) ** -0.5
        # Shape: (num_blocks, block_in, block_out, freq_T) as complex
        # Stored as real with last dim=2
        self.W = nn.Parameter(
            scale * torch.randn(num_blocks, self.block_in, self.block_out, self.freq_T, 2)
        )
        
        # 1x1 spatial-domain bypass for gradient flow
        self.bypass = nn.Conv3d(in_ch, out_ch, kernel_size=1, bias=False)
        self.norm = nn.GroupNorm(8, out_ch)

    def forward(self, x):
        # x: (B, 2C, T, H, W)
        B, C_in, T, H, W = x.shape
        residual = self.bypass(x)  # (B, C, T, H, W)

        # Step 1: temporal FFT
        x_ft = torch.fft.rfft(x, dim=2, norm='ortho')
        # x_ft: (B, 2C, freq_T, H, W) complex

        # Step 2: reshape into blocks
        x_ft = x_ft.view(B, self.num_blocks, self.block_in, self.freq_T, H, W)
        # x_ft: (B, N, I, F, H, W)  where N=num_blocks, I=block_in, F=freq_T

        # Step 3: complex channel mixing per frequency mode
        W_complex = torch.view_as_complex(self.W)
        # W_complex: (N, I, O, F)  where O=block_out

        out_ft = torch.einsum('bnifhw, niof -> bnofhw', x_ft, W_complex)
        # Reads as: for each (b,n,f,h,w), matrix-multiply over i → o
        # out_ft: (B, N, O, F, H, W)

        # Step 4: reshape back to (B, C_out, freq_T, H, W)
        out_ft = out_ft.reshape(B, self.out_ch, self.freq_T, H, W)

        # Step 5: inverse FFT back to time domain
        out = torch.fft.irfft(out_ft, n=T, dim=2, norm='ortho')
        # out: (B, C, T, H, W)
...
class AmpCell(nn.Module):
    def __init__(self, t_in, t_out, dim, weight_scale, alpha, beta, freq_multiplier, size_factor=1.0,
        ...
        self.fusion = SpectralFusion(2*dim, dim, t_out)
