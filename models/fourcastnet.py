...
class AttrDict(dict):
    """Helper to mock the params object expected by AFNONet"""
    def __init__(self, *args, **kwargs):
        super(AttrDict, self).__init__(*args, **kwargs)
        self.__dict__ = self

class FourCastNet_Pipeline(nn.Module):
    def __init__(self, input_seq_len=5, future_seq_len=20, img_size=(128, 128), in_channels=1):
        super(FourCastNet_Pipeline, self).__init__()
        self.input_seq_len = input_seq_len
        self.future_seq_len = future_seq_len
        self.in_channels = in_channels
        
        # We stack all past frames into the channel dimension
        stacked_in_channels = input_seq_len * in_channels
        
        # Configure the params expected by your AFNONet class
        params = AttrDict({
            'patch_size': 8,
            'num_blocks': 8,
            'N_in_channels': stacked_in_channels,
            'N_out_channels': in_channels
        })
        
        # Initialize the backbone
        # Note: You may need to lower embed_dim (e.g., 384) or depth (e.g., 6) 
        # if 768/12 causes Out-Of-Memory errors on your GPU.
        self.afno = AFNONet(
            params=params,
            img_size=img_size,
            patch_size=(8, 8),
            in_chans=stacked_in_channels,
    ...
    def forward(self, past_inputs):
        """
        past_inputs: [Seq_Len, Batch, Channels, Height, Width] (e.g., [5, B, 1, 128, 128])
        """
        S_in, B, C, H, W = past_inputs.size()
        
        # Reshape [S_in, B, C, H, W] -> [B, S_in * C, H, W]
        # We permute the time dimension to sit next to the channel dimension, then flatten them
        x = past_inputs.permute(1, 0, 2, 3, 4).reshape(B, S_in * C, H, W)
        ...
        for _ in range(self.future_seq_len):
            # Predict the next single frame: [B, C, H, W]
            next_frame = self.afno(current_input)
            predictions.append(next_frame)
            
            # Prepare the input for the next step:
            # Drop the oldest frame (the first C channels) and append the new prediction
            current_input = torch.cat([current_input[:, C:, :, :], next_frame], dim=1)
            
        # Stack the list of predictions back into [Seq_Len, Batch, Channels, Height, Width]
        final_predictions = torch.stack(predictions, dim=0)
        
        return final_predictions

class FourCastNet_Model(nn.Module):
    def __init__(self, input_seq_len=5, future_seq_len=20, **kwargs):
        super().__init__()
        # Replace the TrajGRU pipeline with the FourCastNet pipeline
        self.net = FourCastNet_Pipeline(
    ...
    def forward(self, x):
        ...
        out = self.net(x)
        # out is [T, B, C, H, W], revert it back
...
if __name__ == "__main__":
    params = {'num_blocks': 8, 'nettype': 'afno', 'patch_size': 8, 'modes':32, "N_in_channels": 1, 'N_out_channels':1}
    model = AFNONet(params, img_size=(128, 128), patch_size=(4,4), in_chans=1, out_chans=1)
