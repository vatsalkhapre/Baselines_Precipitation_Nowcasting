...
class AmpCell(nn.Module):
    def __init__(self, t_in, t_out, dim, weight_scale, alpha, beta, freq_multiplier, size_factor=1.0,
        ...
        self.conv_spectral = nn.Sequential(ResneSpectralBlock(dim*t_out),
                                     ResneSpectralBlock(dim*t_out),
                                     AFNO2D(dim*t_out))
...
class ResneSpectralBlock(nn.Module):
    def __init__(self, dim, groups = 8): #'zeros', 'reflect', 'replicate' or 'circular'
        super().__init__()
        self.block1 = SpectralBlock_2D(dim, groups = groups)
        self.block2 = SpectralBlock_2D(dim, groups = groups)
