...
class AFNO2D(nn.Module):
    ...
    def forward(self, x):
        ...
        o1_real[:, :, :kept_modes] = F.leaky_relu(
            torch.einsum('...bi,bio->...bo', x[:, :, :kept_modes].real, self.w1[0]) - \
            torch.einsum('...bi,bio->...bo', x[:, :, :kept_modes].imag, self.w1[1]) + \
            self.b1[0], negative_slope=0.5
        )

        o1_imag[:, :, :kept_modes] = F.leaky_relu(
            torch.einsum('...bi,bio->...bo', x[:, :, :kept_modes].imag, self.w1[0]) + \
            torch.einsum('...bi,bio->...bo', x[:, :, :kept_modes].real, self.w1[1]) + \
            self.b1[1], negative_slope=0.5
        )

        o2_real[:, :, :kept_modes] = F.leaky_relu(
            torch.einsum('...bi,bio->...bo', o1_real[:, :, :kept_modes], self.w2[0]) - \
            torch.einsum('...bi,bio->...bo', o1_imag[:, :, :kept_modes], self.w2[1]) + \
            self.b2[0], negative_slope=0.5
        )

        o2_imag[:, :, :kept_modes] =  F.leaky_relu(
...
class AmpCell(nn.Module):
    def __init__(self, t_in, t_out, dim, weight_scale, alpha, beta, freq_multiplier, hidden_size_factor, size_factor=1.0,
        ):
        super().__init__()
        ...
        self.conv_spectral = nn.Sequential(ResneSpectralBlock(dim*t_out, hidden_size_factor),
                                     ResneSpectralBlock(dim*t_out, hidden_size_factor),
                                     AFNO2D(dim*t_out, hidden_size_factor=hidden_size_factor))

        self.afno = AFNO2D(dim*t_out, 1, 1)
    ...
    def forward(self, x):
        ...
        x = self.conv_spectral(x)
        x = rearrange(x, 'b h w (t c) -> b t c h w' , t=self.t_out)
        x = x + residual
        return x

class AmpliNet(nn.Module):
    def __init__(self, pre_seq_length, aft_seq_length, dim, hidden_dim, weight_scale, alpha, beta, freq_multiplier, hidden_size_factor, n_layers=1, mlp_ratio=2):
        super().__init__()
        ...
        self.amplist = nn.ModuleList([
            AmpCell(pre_seq_length if i==0 else aft_seq_length, aft_seq_length, hidden_dim, weight_scale, alpha, beta, freq_multiplier, hidden_size_factor) for i in range(n_layers)
        ])
...
class AlphaPre_Amplinet(nn.Module):
    def __init__(self, weight_scale, alpha, beta, freq_multiplier, hidden_size_factor, total_steps,const_ratio, pre_seq_length, aft_seq_length, input_shape, input_dim, 
        ...
        self.amplinet = AmpliNet(pre_seq_length, aft_seq_length, input_dim, hidden_dim, weight_scale, alpha, beta, freq_multiplier, hidden_size_factor)
...
class SpectralBlock_2D(nn.Module):
    def __init__(self, dim, groups = 8, groupnorm=True):
        super(SpectralBlock_2D, self).__init__()
        self.proj = AFNO2D(dim)
...
class ResneSpectralBlock(nn.Module):
    def __init__(self, dim, hidden_size_factor, groups = 8): #'zeros', 'reflect', 'replicate' or 'circular'
        super().__init__()
        self.block1 = SpectralBlock_2D(dim, hidden_size_factor, groups = groups)
        self.block2 = SpectralBlock_2D(dim, hidden_size_factor, groups = groups)
...
def get_model(
    ...
    **kwargs
    ...
    model = AlphaPre_Amplinet(weight_scale, alpha, beta, freq_multiplier,afno2D_hidden_size_factor, total_steps,const_ratio, pre_seq_length=T_in, aft_seq_length=T_out, input_shape=input_shape, input_dim=img_channels, 
