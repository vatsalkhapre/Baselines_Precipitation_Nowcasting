...
class SpectralBlock_2D(nn.Module):
    def __init__(self, dim, num_blocks, sparsity_threshold, hidden_size_factor,
                 k_spatial, norm_before=True, if_residual=False,
                 adapt_fusion=True, channel_mixing=False,
                 groupnorm=True, groups=8):
    ...
    def forward(self, x):
        ...
        x_spec = self.proj(x_norm.permute(0,2,3,1))
        x_spec = x_spec.permute(0,3,1,2)

        if self.adapt_fusion:
            x_fused = x_spa + self.alpha * x_spec   # ✔ corrected direction
        else:
...
class ResneSpectralBlock(nn.Module):
    def __init__(self, dim,num_blocks, sparsity_threshold, hidden_size_factor,  k_spatial, norm_before, if_residual, adapt_fusion, channel_mixing, groups = 8): #'zeros', 'reflect', 'replicate' or 'circular'
        super().__init__()
        self.block1 = SpectralBlock_2D(dim, num_blocks=num_blocks, sparsity_threshold=sparsity_threshold, hidden_size_factor= hidden_size_factor, k_spatial= k_spatial, norm_before=norm_before, if_residual=if_residual, adapt_fusion=adapt_fusion, channel_mixing=channel_mixing, groups = groups)
        self.block2 = SpectralBlock_2D(dim, num_blocks=num_blocks, sparsity_threshold=sparsity_threshold, hidden_size_factor= hidden_size_factor, k_spatial= k_spatial, norm_before=norm_before, if_residual=if_residual, adapt_fusion=adapt_fusion, channel_mixing=channel_mixing, groups = groups)
...
class WaveletGaborBlock(nn.Module):
    ...
    def __init__(self, t_in, t_out, dim, num_blocks,sparsity_threshold, hidden_size_factor,
                 ...
                 k_spatial, norm_before, if_residual, adapt_fusion, channel_mixing,
        ...
        self.conv_spectral = nn.Sequential(ResneSpectralBlock(dim*t_out, num_blocks, sparsity_threshold, hidden_size_factor, k_spatial, norm_before, if_residual, adapt_fusion, channel_mixing),
                                     ResneSpectralBlock(dim*t_out, num_blocks, sparsity_threshold, hidden_size_factor, k_spatial, norm_before, if_residual, adapt_fusion, channel_mixing),
...
def get_model(
    ...
    size_factor=1.0,
    total_steps=50000, const_ratio=0.5, k_spatial=3, norm_before=False, if_residual=False, adapt_fusion=True, channel_mixing=True,
