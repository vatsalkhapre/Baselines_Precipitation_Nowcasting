...
class AmpCell(nn.Module):
    def __init__(self, t_in, t_out, dim, weight_scale, alpha, beta, freq_multiplier, size_factor=1.0, wave='haar'
        ...
        self.conv_high = nn.Sequential(
            nn.Conv2d(3 * dim * t_out, dim * t_out, kernel_size=1),  # compress
            ResnetBlock(dim * t_out, dim * t_out),
            nn.Conv2d(dim * t_out, 3 * dim * t_out, kernel_size=1),  # expand
        )
    ...
    def forward(self, x):
        ...
        # DWTForward returns:
        #   ll: (B*T, C, H/2, W/2)
        #   hf: list of length J, each element is (B*T, C, 3, H/2, W/2)
        ll, hf_list = self.dwt(x_flat)
        hf = rearrange(hf_list[0], 'bt c n h w -> bt (c n) h w')  # (B*T, 3C, H/2, W/2)
        
        # Flatten time into channels
        ll = rearrange(ll, '(b t) c h w -> b (t c) h w', t=T)
        hf = rearrange(hf, '(b t) c h w -> b (t c) h w', t=T)
        
        # Process separately
        ll = self.conv_low(ll)
        hf = self.conv_high(hf)
