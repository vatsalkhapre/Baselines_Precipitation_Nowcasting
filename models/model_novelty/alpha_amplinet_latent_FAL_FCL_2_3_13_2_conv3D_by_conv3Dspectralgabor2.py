...
class SpectralConv3d(nn.Module):
    def __init__(self, in_channels, out_channels, modes_t, modes_h, modes_w):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        
        # Number of low-frequency modes to keep in Time, Height, and Width
        ...
        # We need 4 complex weight tensors to handle the 4 corners of the 3D frequency space
        # Weights shape: [in_channels, out_channels, modes_t, modes_h, modes_w]
        self.w1 = nn.Parameter(self.scale * torch.rand(in_channels, out_channels, self.modes_t, self.modes_h, self.modes_w, dtype=torch.cfloat))
        self.w2 = nn.Parameter(self.scale * torch.rand(in_channels, out_channels, self.modes_t, self.modes_h, self.modes_w, dtype=torch.cfloat))
        self.w3 = nn.Parameter(self.scale * torch.rand(in_channels, out_channels, self.modes_t, self.modes_h, self.modes_w, dtype=torch.cfloat))
        self.w4 = nn.Parameter(self.scale * torch.rand(in_channels, out_channels, self.modes_t, self.modes_h, self.modes_w, dtype=torch.cfloat))

    def compl_mul3d(self, input, weights):
        # Einstein summation for 3D complex multiplication
        # b: batch, i: in_channel, o: out_channel, t: time, h: height, w: width
        return torch.einsum("bithw, iothw -> bothw", input, weights)

    def forward(self, x):
        # x shape expects: [Batch, Channels, Time, Height, Width]
        batchsize = x.shape[0]

        # 1. 3D Fast Fourier Transform over Time, Height, and Width
        x_ft = torch.fft.rfftn(x, dim=[-3, -2, -1])

        # 2. Initialize output frequency tensor (zeros out high frequencies)
        out_ft = torch.zeros(batchsize, self.out_channels, x.size(-3), x.size(-2), x.size(-1)//2 + 1, dtype=torch.cfloat, device=x.device)

        # 3. Apply complex weights to the 4 corners of the frequency domain
        
        # Corner 1: (Low Time, Low Height)
        out_ft[:, :, :self.modes_t, :self.modes_h, :self.modes_w] = \
            self.compl_mul3d(x_ft[:, :, :self.modes_t, :self.modes_h, :self.modes_w], self.w1)
            
        # Corner 2: (High Time, Low Height)
        out_ft[:, :, -self.modes_t:, :self.modes_h, :self.modes_w] = \
            self.compl_mul3d(x_ft[:, :, -self.modes_t:, :self.modes_h, :self.modes_w], self.w2)
            
        # Corner 3: (Low Time, High Height)
        out_ft[:, :, :self.modes_t, -self.modes_h:, :self.modes_w] = \
            self.compl_mul3d(x_ft[:, :, :self.modes_t, -self.modes_h:, :self.modes_w], self.w3)
            
        # Corner 4: (High Time, High Height)
        out_ft[:, :, -self.modes_t:, -self.modes_h:, :self.modes_w] = \
            self.compl_mul3d(x_ft[:, :, -self.modes_t:, -self.modes_h:, :self.modes_w], self.w4)

        # 4. Inverse 3D FFT back to spatiotemporal pixel space
...
class AmpCell(nn.Module):
    def __init__(self, t_in, t_out, dim, modes, weight_scale, alpha, beta, freq_multiplier, size_factor=1.0,
        ):
        super().__init__()
        ...
        self.spectralconv3D = SpectralConv3d(2*dim, dim,modes, modes, modes)
...
class AmpliNet(nn.Module):
    def __init__(self,modes, pre_seq_length, aft_seq_length, dim, hidden_dim, weight_scale, alpha, beta, freq_multiplier, n_layers=1, mlp_ratio=2):
        super().__init__()
        ...
        self.amplist = nn.ModuleList([
            AmpCell(pre_seq_length if i==0 else aft_seq_length, aft_seq_length, hidden_dim, modes, weight_scale, alpha, beta, freq_multiplier) for i in range(n_layers)
        ])
...
class AlphaPre_Amplinet(nn.Module):
    def __init__(self, weight_scale, alpha, beta, freq_multiplier, total_steps,const_ratio, pre_seq_length, aft_seq_length, input_shape, input_dim, 
                 hidden_dim, spec_num=20, modes=16):
        super(AlphaPre_Amplinet, self).__init__()
        self.amplinet = AmpliNet(modes, pre_seq_length, aft_seq_length, input_dim, hidden_dim, weight_scale, alpha, beta, freq_multiplier)
...
def get_model(
    ...
    **kwargs
    ...
    model = AlphaPre_Amplinet(weight_scale, alpha, beta, freq_multiplier, total_steps,const_ratio, pre_seq_length=T_in, aft_seq_length=T_out, input_shape=input_shape, input_dim=img_channels, 
                     hidden_dim=dim, spec_num=spec_num, modes = modes)
