...
class AmpCell(nn.Module):
    def __init__(self, t_in, t_out, dim, modes, weight_scale, alpha, beta, freq_multiplier, size_factor=1.0,
        ...
        self.conv = nn.Sequential(Resnet_Spectral_Block(dim*t_out, dim*t_out, modes),
                                     Resnet_Spectral_Block(dim*t_out, dim*t_out, modes),
...
class Spectral_Block(nn.Module):
    def __init__(self, dim, dim_out, modes, groups = 8, groupnorm=True):
        super().__init__()
        self.spatial_temporal_interaction = nn.Sequential(
            SpectralConv2d(dim , dim_out , modes1=modes, modes2=modes),
            nn.GroupNorm(groups, dim_out) if groupnorm else nn.BatchNorm2d(dim_out),
            nn.LeakyReLU(0.5)
            )

    def forward(self, x):
        x = self.spatial_temporal_interaction(x)
        return x

class Resnet_Spectral_Block(nn.Module):
    def __init__(self, dim,num_blocks , groups = 8, groupnorm=True): #'zeros', 'reflect', 'replicate' or 'circular'
        super().__init__()
        self.block1 = AFNO2D(dim, , groups = groups, modes=modes)
        self.block2 = Spectral_Block(dim_out, dim_out, groups = groups, modes=modes)
