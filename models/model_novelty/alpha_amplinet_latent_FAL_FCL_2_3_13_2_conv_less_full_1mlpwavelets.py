"""
Wavelet LASTOCast Block — Experiment 1: MLP-only, 1 MLP per band (No Gabor)

Pipeline:
    Input → Lifting → DWT → [MLP per band] → IDWT
          → Spatio-Temporal Conv → + MLP residual → Projection

Changes vs gabor2:
    - GaborLayer removed entirely
    - BandTemporalStream: single MLP only (no Gabor, no fusion conv)
    - MLP residual replaces Gabor residual
...
# ============================================================
# Temporal stream: single MLP for one band
# ============================================================

class BandTemporalStream(nn.Module):
    """Single MLP temporal modeling for one frequency band."""
    ...
    def forward(self, x):
        """
        x:       (B, C, H, W, T_in)
        returns: mlp_out  (B, C, H, W, T_out)   ← for residual path
                 fused    (B, C, T_out, H, W)    ← for IDWT path
        """
...
# ============================================================
# Main Wavelet Block (MLP-only, 1 MLP per band)
# ============================================================
...
class WaveletMLPBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        ll_mlp, ll_fused = self.stream_ll(ll_t)
        ...
        for i, hf in enumerate(hf_list):
            ...
            if self.hf_mode == 'shared':
                hf_mlp, hf_fused = self.stream_hf(hf_t)
            else:
                hf_mlp, hf_fused = self.hf_streams[i](hf_t)
        ...
        # 4. IDWT — MLP residual path
        ll_mlp_flat = rearrange(ll_mlp, 'b c h w t -> (b t) c h w')
        hf_mlp_flat_list = [
            rearrange(hf, 'b (c n) h w t -> (b t) c n h w', n=3)
            for hf in hf_mlp_list
        ]
        mlp_residual = self.idwt((ll_mlp_flat, hf_mlp_flat_list))
...
if __name__ == '__main__':
    print("=== Exp 1: Wavelet MLP-only (1 MLP per band) ===\n")
    configs = [
