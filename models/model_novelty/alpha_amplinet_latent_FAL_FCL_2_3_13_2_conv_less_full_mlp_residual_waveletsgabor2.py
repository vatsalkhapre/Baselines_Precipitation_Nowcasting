...
class WaveletGaborBlock(nn.Module):
    ...
    def _process_j1(self, x):
        ...
        # Temporal streams
        ll_mlp, ll_fused = self.stream_ll(ll)
        hf_mlp, hf_fused = self.stream_hf(hf)
        ...
        ll_mlp_flat = rearrange(ll_mlp, 'b c h w t -> (b t) c h w')
        hf_mlp_flat = rearrange(hf_mlp, 'b (c n) h w t -> (b t) c n h w', n=3)
        mlp_residual = self.idwt((ll_mlp_flat, [hf_mlp_flat]))

        return reconstructed, mlp_residual, H, W
    ...
    def _process_j2_shared(self, x):
        ...
        hf_l1_mlp, hf_l1_fused = self.stream_hf(hf_l1_t)
        hf_l2_mlp, hf_l2_fused = self.stream_hf(hf_l2_t)
        ...
        ll_mlp_flat = rearrange(ll_mlp, 'b c h w t -> (b t) c h w')
        hf_l1_mlp_flat = rearrange(hf_l1_mlp, 'b (c n) h w t -> (b t) c n h w', n=3)
        hf_l2_mlp_flat = rearrange(hf_l2_mlp, 'b (c n) h w t -> (b t) c n h w', n=3)
        mlp_residual = self.idwt((ll_mlp_flat, [hf_l1_mlp_flat, hf_l2_mlp_flat]))
    ...
    def _process_j2_separate(self, x):
        ...
        ll_mlp, ll_fused = self.stream_ll(ll)
        hf_l1_mlp, hf_l1_fused = self.stream_hf_l1(hf_l1_t)
        hf_l2_mlp, hf_l2_fused = self.stream_hf_l2(hf_l2_t)
        ...
        # Gabor residual
        ll_gabor_flat = rearrange(ll_mlp, 'b c h w t -> (b t) c h w')
        hf_l1_gabor_flat = rearrange(hf_l1_mlp, 'b (c n) h w t -> (b t) c n h w', n=3)
        hf_l2_gabor_flat = rearrange(hf_l2_mlp, 'b (c n) h w t -> (b t) c n h w', n=3)
        mlp_residual = self.idwt((ll_gabor_flat, [hf_l1_gabor_flat, hf_l2_gabor_flat]))
    ...
    def forward(self, x):
        ...
        if self.level == 1:
            reconstructed, mlp_residual, H, W = self._process_j1(x)
        elif self.hf_mode == 'shared':
            reconstructed, mlp_residual, H, W = self._process_j2_shared(x)
        else:
            reconstructed, mlp_residual, H, W = self._process_j2_separate(x)

        # Trim to original spatial size
        if reconstructed.shape[-2] != H or mlp_residual.shape[-2] != H:
            raise AssertionError(f"Height mismatch: expected {H}")
        if reconstructed.shape[-1] != W or mlp_residual.shape[-1] != W:
            raise AssertionError(f"Width mismatch: expected {W}")
