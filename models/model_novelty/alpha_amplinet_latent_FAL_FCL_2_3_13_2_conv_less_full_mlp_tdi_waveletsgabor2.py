...
# ============================================================
# Temporal Difference Injector
# ============================================================

class TemporalDifferenceInjector(nn.Module):
    def __init__(self, tin, channels):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x_diff_flat = rearrange(x_diff, 'b t c h w -> b (t c) h w')
        x = rearrange(x, 'b t c h w -> b (t c) h w')
        motion_feat = self.motion_conv(x_diff_flat)
        out = x + self.motion_gate * motion_feat
        out = rearrange(out, 'b (t c) h w -> b t c h w', t=x_seq.shape[1])
        return out
...
class WaveletGaborBlock(nn.Module):
    ...
    def __init__(self, t_in, t_out, dim,
        ...
        # -------- TDI --------
        self.tdi = TemporalDifferenceInjector(t_in, dim)
    ...
    def forward(self, x):
        ...
        # ============================================================
        # 1. Temporal differnce injection
        # ============================================================
        x= self.tdi(x)
        # ============================================================
        # 1. DWT decomposition
