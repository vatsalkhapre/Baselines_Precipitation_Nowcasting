...
class AmpCell(nn.Module):
    def __init__(self, t_in, t_out, dim, weight_scale, alpha, beta, freq_multiplier, size_factor=1.0, wave='haar'
        ...
        # High-freq channel alignment: 3C → C for Gabor, then C → 3C back
        self.hf_compress = nn.Conv2d(3 * dim, dim, kernel_size=1)
        self.hf_expand = nn.Conv2d(dim, 3 * dim, kernel_size=1)
    ...
    def forward(self, x):
        ...
        hf = rearrange(hf_list[0], 'bt c n h w -> bt (c n) h w')
        # ll: (B*T, C, H/2, W/2), hf: (B*T, 3C, H/2, W/2)
        
        # Compress high-freq channels: 3C → C so Gabor can process it
        hf_compressed = self.hf_compress(hf)  # (B*T, C, H/2, W/2)
        
        # Reshape for temporal Gabor
        ll = rearrange(ll, '(b t) c h w -> b c h w t', t=T)
        hf_compressed = rearrange(hf_compressed, '(b t) c h w -> b c h w t', t=T)
        
        # ========== Dual Gabor temporal processing ==========
        ll_out = self.gabor_low(ll)      # (B, C, H/2, W/2, T_out)
        hf_out = self.gabor_high(hf_compressed)  # (B, C, H/2, W/2, T_out)
        
        ll_out = rearrange(ll_out, 'b c h w t -> (b t) c h w', t=self.t_out)
        hf_out = rearrange(hf_out, 'b c h w t -> (b t) c h w', t=self.t_out)
        
        # Expand high-freq back: C → 3C
        hf_out = self.hf_expand(hf_out)  # (B*T_out, 3C, H/2, W/2)
        hf_out = rearrange(hf_out, 'bt (c n) h w -> bt c n h w', n=3)
        
        # ========== Inverse wavelet ==========
        gabor_result = self.idwt((ll_out, [hf_out]))  # (B*T_out, C, H, W)
        gabor_result = rearrange(gabor_result, '(b t) c h w -> b t c h w', t=self.t_out)
        
        # ========== MLP path (full resolution) ==========
        mlp_result = self.tmlp(x.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)
        
        # ========== Fusion + spatial conv (same as before) ==========
        out = torch.cat([gabor_result, mlp_result], dim=2)
        ...
        x = x + gabor_result  # residual from wavelet-Gabor path
        return x
