...
class S_T_Block(nn.Module):
    def __init__(self, dim, dim_out, groups=8, kernel_size=3, padding_mode='zeros'):
        super().__init__()
        self.proj = nn.Conv2d(dim, dim_out, kernel_size=kernel_size,
                              padding=kernel_size // 2, padding_mode=padding_mode, groups=groups)
...
class S_T_TransformBlock(nn.Module):
    def __init__(self, dim, dim_out, groups=2, kernel_size=3, padding_mode='zeros'):
        super().__init__()
        self.block1 = S_T_Block(dim, dim_out, groups=groups,
                            kernel_size=kernel_size, padding_mode=padding_mode)
        self.block2 = S_T_Block(dim_out, dim_out, groups=groups,
                            kernel_size=kernel_size, padding_mode=padding_mode)
        self.skip = nn.Conv2d(dim, dim_out, 1) if dim != dim_out else nn.Identity()
...
class WaveletGaborBlock(nn.Module):
    ...
    """
    def __init__(self, t_in, t_out, dim, groups, 
        ...
        self.spatial_temporal = nn.Sequential(
            S_T_TransformBlock(dim * t_out, dim * t_out, groups),
            S_T_TransformBlock(dim * t_out, dim * t_out, groups),
...
class WaveletLASTOCast(nn.Module):
    def __init__(self, T_in, T_out, in_dim, hidden_dim, st_conv_groups,
                 weight_scale_low, alpha_low, beta_low, freq_multiplier_low,
                 weight_scale_high, alpha_high, beta_high, freq_multiplier_high,
                 size_factor=1.0, wave='haar', level=1, hf_mode='shared'):
        super().__init__()
        ...
        self.operator = WaveletGaborBlock(
            T_in, T_out, hidden_dim, st_conv_groups, 
            weight_scale_low, alpha_low, beta_low, freq_multiplier_low,
            weight_scale_high, alpha_high, beta_high, freq_multiplier_high,
...
class WaveletLASTOCastForecaster(nn.Module):
    def __init__(self, T_in, T_out, in_dim, hidden_dim, st_conv_groups,
        ...
        self.lastocast = WaveletLASTOCast(
            T_in, T_out, in_dim, hidden_dim, st_conv_groups, 
            weight_scale_low, alpha_low, beta_low, freq_multiplier_low,
            weight_scale_high, alpha_high, beta_high, freq_multiplier_high,
            size_factor, wave, level, hf_mode,
        )
...
def get_model(
    ...
    T_in=5, T_out=20,
    wave='haar', wavelet_level=1, hf_mode='shared',st_conv_groups=1, 
    input_shape=(128, 128),
    **kwargs
):
    model = WaveletLASTOCastForecaster(
        T_in=T_in, T_out=T_out,
        in_dim=img_channels, hidden_dim=dim, st_conv_groups=st_conv_groups, 
