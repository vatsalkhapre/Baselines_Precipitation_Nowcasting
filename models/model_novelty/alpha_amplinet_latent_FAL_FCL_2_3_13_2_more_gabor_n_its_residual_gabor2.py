...
class AmpCell(nn.Module):
    def __init__(self, t_in, t_out, dim, weight_scale, alpha, beta, freq_multiplier, size_factor=1.0,
        ...
        self.gabor2 = GaborLayer(t_out, t_out, weight_scale, alpha, beta, freq_multiplier)
        self.tmlp = nn.Sequential(
            nn.Linear(t_in, int(t_out*size_factor)),
            nn.SELU(True),
            nn.Linear(int(t_out*size_factor), t_out),
    ...
    def forward(self, x):
        ...
        residual3 = self.gabor2(x).permute(0,4,1,2,3)
        x = rearrange(residual3, 'b t c h w -> b (t c) h w')
        x = self.conv(x)
