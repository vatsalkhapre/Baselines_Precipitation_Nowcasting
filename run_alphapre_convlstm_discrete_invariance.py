...
def create_parser():
    ...
    parser.add_argument('--backbone',       type=str,   default='our_model',        help='backbone model for deterministic prediction (alphapre/convlstm_paper/simvp)')
    ...
    # --------------- Training ---------------
    parser.add_argument("--batch_size",     type=int,   default=1,               help="batch size")                 #Check
...
class Runner(object):
    ...
    def _build_model(self):
        ...
        if self.args.backbone == "our_model":
            from models.Best_model.alpha_amplinet_latent_FAL_FCL_2_3_13_2_gabor2 import get_model
            kwargs = {
    ...
    def _sample_batch(self, batch, use_ema=False, vis_diff=False):
        ...
        B,T,C,H,W = radar_input.shape
        radar_input = radar_input.reshape(B*T, C, H, W)
        radar_input = self.encode_stage(self.ae, radar_input,1.0)
        std_value = radar_input.std()
        radar_input = radar_input/std_value
        radar_input = rearrange(radar_input, '(b t) c h w -> b t c h w', t = self.args.frames_in)
        radar_pred, *_ = sample_fn(radar_input, compute_loss=False)
        radar_pred = radar_pred*std_value
        radar_pred = rearrange(radar_pred, 'b t c h w -> (b t) c h w', t = self.args.frames_out)
        radar_pred = self.decode_stage(self.ae, radar_pred,1.0)
        radar_pred = rearrange(radar_pred, '(b t) c h w -> b t c h w', t = self.args.frames_out)
