...
def denorm_and_colorize(seq, pixel_scale, gray2color_fn, data_type='vil'):
    """
    seq           : np.ndarray (T, H, W), values in [0, 1]
    pixel_scale   : float
    gray2color_fn : callable — dataset-specific gray2color
    Returns       : np.ndarray (T, H, W, 4), RGBA float64
    """
...
def subsample_frames(seq, target_count=10):
    ...
    T <= target_count: return all frames unchanged
    seq : np.ndarray (T, ...)
    """
    T = seq.shape[0]
    if T <= target_count:
...
def resolve_plot_dir(ckpt_milestone_path):
    """
    .../Exps/<exp_dir>/<exp_name>/checkpoints/ckpt-best.pt
    ->  .../Exps/<exp_dir>/<exp_name>/plots/
    """
...
def create_parser():
    ...
    parser.add_argument("--plot_stride",      type=int,       default=4,         help="Plot every N-th test batch")
...
class Runner(object):
    ...
    def _load_data(self):
        ...
        self.gray2color_fn   = color_save_fn.keywords['gray2color']   # <<< PLOT
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.global_epochs):
            ...
            self.model.train()
            # ---- epoch-level accumulators (logged to wandb at end of epoch) ----    
            epoch_loss_sum = 0.0     #For gradient accumulation 
            epoch_steps    = 0       #For gradient accumulation 
            ...
            for i, batch in enumerate(tqdm(self.train_loader, total=len(self.train_loader))):
                ...
                grad_norm = None     #For gradient accumulation 
                if self.accelerator.sync_gradients:      #For gradient accumulation 
                    grad_norm = self.accelerator.clip_grad_norm_(self.model.parameters(), 1.0)    #For gradient accumulation 
                ...
                if grad_norm is not None:               #For gradient accumulation 
                    log_dict['grad_norm'] = grad_norm.item() if isinstance(grad_norm, torch.Tensor) else float(grad_norm)   #For gradient accumulation 
                ...
                for k,v in loss_dict.items():
                    if type(v) == float:
                        ...
                        log_dict[k] = v.item()
                epoch_loss_sum += log_dict.get('total_loss', 0.0)           #For gradient accumulation 
                epoch_steps    += 1                                         #For gradient accumulation 
        ...
        for batch_idx, (batch, os_batch) in enumerate(tqdm(data_loaders, total=total)):
            ...
            if do_plot and self.is_main and (batch_idx % plot_stride == 0):      # <<< PLOT
                ...
                for b in range(B):                                               # <<< PLOT
                    ...
                    inp = radar_os_input_np[b].squeeze(1)                        # <<< PLOT
