CUDA_VISIBLE_DEVICES=0 python3 run_alphapre_convlstm_sevir_lr_latent.py \
                --backbone amplinet_latent_falfcl_only_2_3_13_2_3Dconv \
                --dataset sevir_lr_latent_32 \
                --exp_dir sevir_lr_latent_32_model_parts \
                --exp_note "amplinet_latent_falfcl_only_sevir_2_3_13_2_3Dconv_corrected" \
                --epochs 50 \
                --ae_ckpt_path "/home/vatsal/NWM/Baselines_Precipitation_Nowcasting/Pretrained_ae_checkpoints/autoencoder_checkpoint_32_sevir.pth" \
                --valid \
                --seq_len 25 \
                --falfcl_weight 1 \
                --frames_in 5 \
                --frames_out 20 \
                --num_workers 8 \
                --wandb_state 'online' \
                --wandb_project_name 'Alphapre' \
                --run_name amplinet_latent_falfcl_only_2_3_13_2_3Dconv_sevir_${weight_scale}_${a}_${b}_${f}

CUDA_VISIBLE_DEVICES=0 python3 run_alphapre_convlstm_sevir_lr_latent.py \
                --backbone amplinet_latent_falfcl_only_2_3_13_2_3Dconv \
                --dataset sevir_lr_latent_32 \
                --exp_dir sevir_lr_latent_32_model_parts \
                --exp_note "amplinet_latent_falfcl_only_sevir_2_3_13_2_3Dconv_corrected" \
                --ae_ckpt_path "/home/vatsal/NWM/Baselines_Precipitation_Nowcasting/Pretrained_ae_checkpoints/autoencoder_checkpoint_32_sevir.pth" \
                --eval \
                --seq_len 25 \
                --falfcl_weight 1 \
                --frames_in 5 \
                --frames_out 20 \
                --num_workers 8 \
                --wandb_state 'offline'