...
def load_dawncast(device='cuda'):
    ...
    model = get_model(
        afno_blocks=1, afno_hidden_size_factor=1,sparsity_threshold=0.01
        total_steps=20000, const_ratio=0.5,
        img_channels=4, dim=64,
        T_in= TIN, T_out=TOUT,
        input_shape=(32, 32), wave='db6', wavelet_level=2, hf_mode='separate'
    )
    # Input shape: (B, T_in, C, H, W) in latent space
    dummy = torch.randn(1, 5, 4, 32, 32)
    return model, dummy, "DAWNCast (Ours)"
...
def load_dawncast_pixel(device='cuda'):
    from models.DAWNCast.dawncast import get_model
    model = get_model(
        afno_blocks=1, afno_hidden_size_factor=1,sparsity_threshold=0.01
        total_steps=20000, const_ratio=0.5,
    ...
    return model, dummy, "DAWNCast (pixel)"
...
def load_lastocast_e2e(device='cuda'):
    ...
    dawncast = dawncast_wrapper.dawncast  # unwrap the forecaster
    ...
    e2e = E2EWrapper(ae, dawncast, scale_factor)
