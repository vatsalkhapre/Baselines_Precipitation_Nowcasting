...
class Evaluator(object):
    def __init__(self,  seq_len, value_scale, thresholds, **kwargs):
        self.metrics = {}
    ...
    def cal_batch_lpips(self, preds, trues):
        ...
        lpips_seq2 = np.array(lpips_seq).squeeze()
      
        if np.array(lpips_seq).shape[1]==1:
            lpips_seq2 = np.expand_dims(lpips_seq2, axis=1)
        # print(f"Shape of lpips seq: {lpips_seq.shape}")
        lpips_seq2 = lpips_seq2.transpose(1,0)
        lpips_batch = list(lpips_seq2)
        return lpips_batch
    ...
    def done(self):
        ...
        maes = np.mean(np.array(self.losses['mae']), axis=0)
        ...
        res_dict['psnr'] = np.mean(psnrs)
        res_dict['ssim'] = np.mean(ssims)
        res_dict['crps'] = np.mean(crpss)
        res_dict['lpips'] = np.mean(lpipss)
        
        print_log('='*20 + f"Losses with {self.seq_len} seq_len"+'='*20)
        print_log(f'<MSE> : {np.mean(mses)}; '+str(mses))
        print_log(f'<MAE> : {np.mean(maes)}; '+str(maes))
