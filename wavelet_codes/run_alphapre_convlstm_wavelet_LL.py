...
def create_parser():
    ...
    parser.add_argument('--backbone',       type=str,   default='amplinet_mseonly',        help='backbone model for deterministic prediction (alphapre/convlstm_paper/simvp)')
    ...
    parser.add_argument("--wandb_project_name", type=str, default="Amplinet_wavelet_LL", help="wandb project name")
    parser.add_argument("--run_name",       type=str,   default='Training_Amplinet_wavelet_LL',        help="wandb run name")            #Check
...
class Runner(object):
    ...
    def _train_batch(self, batch):
        ...
        frames_in_l, _ = self.dwt(frames_in)
        frames_out_l, _ = self.dwt(frames_out)

        frames_in_l = normalize(self.args.dataset, frames_in_l)
        frames_out_l = normalize(self.args.dataset, frames_out_l)

       
        frames_in_l = frames_in_l.view(B, self.args.frames_in, self.args.img_channel, self.args.img_size, self.args.img_size)
        frames_out_l = frames_out_l.view(B, self.args.frames_out, self.args.img_channel, self.args.img_size, self.args.img_size)

        assert radar_batch.shape[1] == self.args.frames_out + self.args.frames_in, "radar sequence length error"
        
        if hasattr(self.model, 'module'):
            _, loss = self.model.module.predict(frames_in=frames_in_l, frames_gt=frames_out_l, compute_loss=True)
        else:
            _, loss = self.model.predict(frames_in=frames_in_l, frames_gt=frames_out_l, compute_loss=True)
    ...
    def _sample_batch(self, batch, use_ema=False, vis_diff=False):
        ...
        radar_input_l, _ = self.dwt(radar_input)
        radar_input_l = radar_input_l.view(B, self.args.frames_in, self.args.img_channel, self.args.img_size, self.args.img_size)
        radar_gt_l, _ = self.dwt(radar_gt)
        radar_gt_l = radar_gt_l.view(B, self.args.frames_out, self.args.img_channel, self.args.img_size, self.args.img_size)

        radar_input_l = normalize(self.args.dataset, radar_input_l)
        radar_gt_l = normalize(self.args.dataset, radar_gt_l)

        radar_pred, *_ = sample_fn(radar_input_l,compute_loss=False)
